    ...
    */
    filterLetter(query?: string): void {
        if (!query) {
            ...
            const newUrl = this.location.path().split('?')[0] + `?search=${query}&page=1&limit=10`;
    ...
    navigationHelper(): void {
        const newUrl = this.location.path().split('?')[0] + `?limit=10&page=1`;
        this.location.replaceState(newUrl);
        this.adminService.getUsers(1).subscribe(res => {
            this.adminDataSource = res
        });
    }
