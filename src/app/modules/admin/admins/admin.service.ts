...
export class AdminService {
    ...
    getUsers(pageNumber: number = 1, search?: string): Observable<{ rows: any[], pagination: { page: number, total: number } }> {
        return this.http.get('api/v1/users', {
            ...
            tap((res: any) => {
                this._users.next({
                    rows: res,
                    pagination: { page: pageNumber ? pageNumber : 1, total: res?.length }
                })
            }),
            catchError(err => {
                return of(err);
