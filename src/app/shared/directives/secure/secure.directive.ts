...
export class SecureDirective {
    ...
    ngOnInit() {
        this.authSubscription = this.authServie.getUserRole().subscribe(res => {
            ...
            const superRole = roles.find(item => item === 'super_admin' || item === 'operation_manager');
            const isSuperAdmin = superRole == Role.superAdmin || superRole == Role.operationManager ? true : false;
